...
1. Install the `minikube` in any way possible (if required). Linux Mint example:
   ```shell
...
3. Start kubernetes in Docker with `minikube`:
   ...
   ```
4. Install the [nginx ingress controller](https://kubernetes.github.io/ingress-nginx/deploy/):
   ```shell
...
5. Configure `minikube`:
   ...
   ```
6. Download project to a pre-created directory:
   ```shell
